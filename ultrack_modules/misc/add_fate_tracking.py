...
def add_fate_to_ultrack_table(ultrack_table : DataFrame) -> DataFrame:
    ...
    last_frame_ids = new_table[new_table["t"] == max_frame].track_ids.unique()

    for id in ultrack_table.track_id.unique():
        if id in parent_ids:
            new_table[new_table.track_id == id]["fate"] = "mitosis"
        elif id in last_frame_ids:
            new_table[new_table.track_id == id]["fate"] = "lived"
        else:
            new_table[new_table.track_id == id]["fate"] = "death"
