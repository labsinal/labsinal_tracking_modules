...
def save_image(open_image:ndarray,
    ...
    :return: None
    """
    save_name = image_name.replace(".tif", "_overlay.png")
...
def add_overlay_to_first_frame(open_image:ndarray,
    ...
    for id in current_ids:
        ...
        if print_index:
            ...
            # Get the cell point
            current_point = (first_frame_df[first_frame_df["ID"] == id]["x"],
                             first_frame_df[first_frame_df["ID"] == id]["y"])
...
def add_overlay_to_frame(tracking_df:DataFrame,
    ...
    :return: None
    """
    current_ids = tracking_df[tracking_df["t"] == n_frame]["ID"]
    for id in current_ids:
        current_df = tracking_df[tracking_df["t"] <= n_frame]
        cell_df = current_df[current_df["ID"] == id]
...
def add_trackings_overlays(images_folder:str, 
    ...
    # Get frames num
    frames_num = tracking_df["t"].max()
    
    # Iterating over frames
    for n_frame in range(int(frames_num)+1):
            
        #open image
        open_image = imread(join(images_folder, images[n_frame]))
